...
export default function ApplicantDetail() {
  ...
  const [allocForm,   setAllocForm]   = useState({
    institutionCode: '', programCode: '', courseType: 'UG',
  ...
  const allocateSeat = async () => {
    if (!allocForm.institutionCode.trim()) {
      setError('Institution Code is required'); return
    }
    setSaving('alloc')
    try {
      ...
      await admissionApi.allocate({
        ...
        institutionCode: allocForm.institutionCode.trim().toUpperCase(),
        programCode:     (allocForm.programCode.trim() || prog?.code || 'NA').toUpperCase(),
      ...
      <Dialog open={allocDialog} onClose={() => setAllocDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ pt:'16px !important', display:'flex', flexDirection:'column', gap:2 }}>
          ...
          <Box sx={{ display:'flex', gap:2 }}>
            <TextField
              label="Institution Code *" value={allocForm.institutionCode}
              onChange={e => setAllocForm(p => ({ ...p, institutionCode: e.target.value }))}
              fullWidth placeholder="e.g. RNSIT" />
            <TextField
              label="Program Code" value={allocForm.programCode}
              onChange={e => setAllocForm(p => ({ ...p, programCode: e.target.value }))}
              fullWidth placeholder="e.g. CSE" />
          </Box>
          <Box sx={{ display:'flex', gap:2 }}>
          ...
          <TextField
            label="Processed By" value={allocForm.processedBy}
...
// import React, { useState, useEffect } from 'react'
// import {
//   Box, Card, Grid2 as Grid, Typography, Button, Chip, Alert, CircularProgress,
//   Table, TableBody, TableCell, TableContainer, TableHead, TableRow,
//   MenuItem, TextField, Dialog, DialogTitle, DialogContent, DialogActions,
//   Divider, Avatar, IconButton, Tooltip, LinearProgress
// } from '@mui/material'
...
//   // ── Document update ────────────────────────────────────────────────────────
//   // FIX: sends PATCH (not PUT) with correct field names matching UpdateDocumentRequest
//   const updateDoc = async () => {
//     if (!docDialog.doc) return
//     setSaving('doc')
//     try {
//       await applicantApi.updateDocument(id, docDialog.doc.id, {
//         status:     docForm.status,      // DocumentChecklist.DocumentStatus enum
//         remarks:    docForm.remarks || null,
//         verifiedBy: docForm.status === 'VERIFIED' ? docForm.verifiedBy : null,
//       })
//       setDocDialog({ open: false, doc: null })
...
//   // ── Mark fee as PAID ───────────────────────────────────────────────────────
//   // FIX: calls PATCH /fees/applicant/{id}
//   const markFeePaid = async () => {
//     setSaving('feePay')
//     try {
//       await feeApi.update(id, {
